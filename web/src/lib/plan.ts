...
export async function enforceQuotaOrExplain(supa: any, widgetId: string, kind = "message") {
  ...
  if (info.plan === "pro") {
    ...
    const { error } = await supa.rpc("inc_usage", {
      p_widget_id: widgetId,
      p_period: info.period,
      p_kind: kind,
    });
    return { ok: true, info };
  }
  ...
  // increment
  const { data, error } = await supa.rpc("inc_usage", {
    p_widget_id: widgetId,
    p_period: info.period,
    p_kind: kind,
  });
  ...
  return {
    ok: !exceeded,
    info: { ...info, count: newCount, remaining: Math.max(0, info.limit - newCount), exceeded },
  };
}
