'use client';
/** Edit Business Profile — restored (2025-11-01_09-50-19)
 * - Client component to avoid server crashes
 * - Country dropdown (all countries via Intl.DisplayNames)
 * - Dial code input with common-code suggestions (datalist) — user can enter ANY code
 * - Working hours grid (Mon–Sun) with time inputs + Closed toggle
        ...
        <form onSubmit={save} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {DAYS.map(({ key, label }) => {
                const row = (biz.hours ?? defaultHours())[key] ?? { open: '09:00', close: '18:00', closed: false };
                return (
                  <div key={key} className="grid grid-cols-[60px_120px_24px_120px_auto] items-center gap-3">
                    ...
                    <input
                      ...
                      required={!row.closed}
                    />
                    <span className="text-center opacity-70">—</span>
                    <input
                      type="time"
                      className="bg-slate-900 border border-slate-700 rounded px-2 py-1"
                      value={row.close}
