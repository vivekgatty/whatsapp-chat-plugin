...
export default function BusinessForm({ userId, initial }: { userId: string; initial: any | null }) {
  ...
  const [hours, setHours] = React.useState<WorkingHours>(initial?.working_hours ?? DEFAULT_HOURS);
  ...
  return (
    <form onSubmit={onSave} className="space-y-4">
      ...
      <div className="rounded-lg border border-zinc-700 p-4">
        <p className="mb-3 font-medium">Working hours</p>
        <div className="space-y-2">
          {(Object.keys(DAY_LABELS) as (keyof WorkingHours)[]).map((day) => (
            <div key={day} className="grid grid-cols-[3rem,auto,auto,auto] items-center gap-3">
