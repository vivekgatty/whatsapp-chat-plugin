...
export async function POST(req: NextRequest) {
  ...
  if (!okType.includes(type)) return NextResponse.json({ ok:false, error: "unsupported_type" }, { status: 400 });

  // ≤ 2MB
  if (file.size > 2 * 1024 * 1024) return NextResponse.json({ ok:false, error: "too_big" }, { status: 400 });

  // bucket (idempotent)
  try { await admin.storage.createBucket(BUCKET, { public: true, fileSizeLimit: "2MB" }); } catch {}

  const safeName = (file.name || "logo").replace(/[^\w.\-]+/g, "_");
  const ext = safeName.includes(".") ? safeName.split(".").pop() : (type === "image/png" ? "png" : type === "image/jpeg" ? "jpg" : type === "image/webp" ? "webp" : "svg");
  const key = `${user.id}/logo.${ext}`;

  const up = await admin.storage.from(BUCKET).upload(key, file, { upsert: true, contentType: type || undefined });
